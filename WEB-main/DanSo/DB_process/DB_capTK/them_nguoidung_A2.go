...
func ThemTaiKhoanA2ToDB(request *models.ThemNguoiDungRequest) error {
	db := database.Connect()
	defer db.Close()

	rowsTinh, err := db.Query("SELECT * FROM tinh where id_tinh = ?", request.TaiKhoan)
	if err != nil {
	...
	if !rowsTinh.Next() {
		return fmt.Errorf("Không thể cấp tài khoản cho Tỉnh không tồn tại")
	}
