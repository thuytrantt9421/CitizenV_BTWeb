...
func ThemTaiKhoanB2ToDB(request *models.ThemNguoiDungRequest) error {
	db := database.Connect()
	defer db.Close()

	rowsTinh, err := db.Query("SELECT * FROM xom where id_xom = ?", request.TaiKhoan)
	if err != nil {
	...
	if !rowsTinh.Next() {
		return fmt.Errorf("Không thể cấp tài khoản cho Xóm không tồn tại")
	}
